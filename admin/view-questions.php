                                ...
                                <div class="panel panel-white">
                                    <div class="panel-body">
                                        <div class="tabs-below" role="tabpanel">
											...
											<?php 
											include '../database/config.php';
											// $sql = "SELECT * FROM tbl_questions WHERE exam_id = '$exam_id' ORDER BY serialno ASC";
                                            $sql = "SELECT * FROM tbl_questions tq LEFT JOIN tbl_question_set tqs ON tq.question_id=tqs.questionid WHERE tq.exam_id='$exam_id' OR tqs.examid='$exam_id'";
                                            $result = $conn->query($sql);
                                            ...
                                            <div role="tabpanel" class="active tab-pane fade in" id="tab<?= $qno ?>">
                                            <p class="row pull-right"><a href="#tab<?= $qno-1 ?>" onclick="next('<?= $qno-1 ?>')" role="tab" data-toggle="tab" class="btn btn-success"><< Prev</a> <a href="#tab<?= $qno+1 ?>" onclick="next('<?= $qno+1 ?>')" role="tab" data-toggle="tab" class="btn btn-success">Next >></a></p>
                                             <p><b>Question <?= $qno ?>:</b> <?= $qs ?></p>
                                             ...
                                             <p><strong>Answer: <span class="text-primary"><?= $row['answer'] ?></span></strong></p>
                                             <h5>Explanation</h5>
                                             <p><?= $row['explanation']; ?></p>
                                             <hr>
                                            ...
                                            <?php
                                            print '
											 <a target="_blank" class="btn btn-twitter m-b-xs" href="edit-question.php?id='.$row['question_id'].'"><i class="fa fa-pencil"></i></a>
                                            ...
                                            while($row = $result->fetch_assoc()) {
											if ($qno == "1") {
											print '<li role="presentation" class="active"><a href="#tab'.$qno.'" role="tab" id="i'.$qno.'"  data-toggle="tab">Q'.$qno.'</a></li>';	
											}else{
											print '<li role="presentation"><a href="#tab'.$qno.'" role="tab" id="i'.$qno.'" data-toggle="tab">Q'.$qno.'</a></li>';		
											}

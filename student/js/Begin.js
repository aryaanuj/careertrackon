var myInterval, AttemptedAns = [],
  ...
  $(document).ready(function(){
    ...
    alert(stuid);
    alert(subj);
    alert(stuid+" "+subj+" "+examid+" "+exam_desc+" "+sdate);
            });
...
function CheckResult() {
    ...
    var scorePercentage = ((Math.round(score * 100) / (TotalQuestion*4)).toFixed(2));
    ...
    $('#lblRScore').text(scorePercentage+"%");
    var stuid = $(".stuid").val();
    var subj = $(".subj").val();
    var examid = $(".examid").val();
    var exam_desc = $(".exam_desc").val();
    ...
    $.ajax({
        url:"answerSave.php",
        method:"post",
        data:{TotalQuestion:TotalQuestion,TotalAttempted:TotalAttempted,TotalCorrect:TotalCorrect,
            TotalWrong:TotalWrong, scorePercentage:scorePercentage, stuid:stuid, subj:subj, exam_id:examid, exam_desc:exam_desc, sdate:sdate },
...
$(document).ready(function() {
    ...
    $("#btnYesSubmit").on("click", function(e) {
        e.preventDefault(); $(".exam-confirm").show(); $("#divdrplngcng").hide(); $(".exam-summery").hide(); $(".timer-title").text("Time Out");
        CleartTimer();
    });
